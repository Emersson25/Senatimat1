...
if (isset($_POST['operacion'])){
  ...
  if ($_POST['operacion'] == 'registrar'){
    ...
    //Vamos a verificar si la vista nos envió 
    if (isset($_FILES['cv'])){
  ...
  if ($_POST['operacion'] == 'listar'){
    ...
    if ($data){
      $numeroFila = 1;
      $datosColaboradores = '';
      $botonNulo = " <a href='#' class='btn btn-sm btn-warning' title='No hay ningun documento'><i class='bi bi-eye-slash-fill'></i></a>";
      ...
      foreach($data as $registro){
        ...
        if ($registro['cv'] == ''){
          ...
          echo " <a href='../views/document/pdf/{$registro['cv']}' data-lightbox='{$registro['idcolaborador']}' data-title='{$datosColaboradores}' class='btn btn-sm btn-warning'><i class='bi bi-eye-fill'></i></a>";
        }
